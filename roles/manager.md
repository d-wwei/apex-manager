...
- 每次 `worker spawn`、`orch start`、或批量 kickoff 后，必须在 5-10 秒内做一次 smoke test：至少读一次 worker terminal/surface，或执行一次 `apex-manager worker status <task-id>` / `list` 抽检，确认 worker 真的启动而不是只拿到了 0 退出码。
