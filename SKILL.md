...
5. Within 5-10 seconds, verify real launch activity, not just a visible prompt: look for `task claim`, `status.json`, `result.json`, or `apex-manager worker status <id>` reporting a verified launch
...
- Treat `.apex-manager/workers/<task-id>/worker-protocol.md` as the canonical worker protocol path. Do not assume a shared `.apex-manager/worker-protocol.md`.
- On non-git directories, call out that Apex Manager is running in shared project-root mode with no worktree isolation before spawning parallel workers.
- When checking a spawned worker, validate three layers when possible: worker meta, terminal content, and tmux/cmux client visibility.
